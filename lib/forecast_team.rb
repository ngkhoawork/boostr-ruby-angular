class ForecastTeam
  ...
  def complete_deals
    @complete_deals ||= Deal.joins(:deal_members).where("deal_members.user_id in (?)", all_teammembers.map{|member| member.id}).active.at_percent(100).closed_at(start_date, end_date)
  end

  def incomplete_deals
    @incomplete_deals ||= Deal.joins(:deal_members).where("deal_members.user_id in (?)", all_teammembers.map{|member| member.id}).active.closed.at_percent(0).closed_at(start_date, end_date)
  end
