class ProductForecastMember
  include ActiveModel::SerializerSupport

  delegate :id, to: :member
  delegate :name, to: :member

  attr_accessor :member, :product, :start_date, :end_date, :quarter, :year

  def initialize(member, product, start_date, end_date, quarter = nil, year = nil)
    self.member = member
  ...
  def revenue
    ...
    @revenue = ios.sum do |io|
      io.effective_product_revenue_budget(member, product, start_date, end_date)
    end
  end
