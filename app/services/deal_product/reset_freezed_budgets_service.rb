class DealProduct::ResetFreezedBudgetsService
  ...
  def reset_edge_products_date
    ...
    first_budget = deal_product_budgets.for_year_month(start_edge_date)&.first
    end_budget = deal_product_budgets.for_year_month(end_edge_date)&.first
  ...
  def remove_budget(month)
    deal_product_budgets.for_year_month(month).destroy_all
  end
  ...
  def start_date
    @_start_date ||= deal.start_date
  end

  def end_date
    @_end_date ||= deal.end_date
  end
  ...
  def first_product_date
    @_first_product_date ||= deal_product_budgets.first.start_date
  end

  def end_product_date
    @_end_product_date ||= deal_product_budgets.last.end_date
  end
