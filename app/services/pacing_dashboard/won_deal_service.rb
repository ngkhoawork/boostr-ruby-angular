class PacingDashboard::WonDealService < PacingDashboard::BaseService
	def perform
		{
	...
	def won_current_quarter_series
		weeks_for_current_quarter.each_with_object({}) do |week, memo|
			...
			won_deals_for_current_quarter.map do |key, value|
				memo[week.start_date] += value.to_i if week.start_date.beginning_of_day <= key && week.end_date.end_of_day >= key
			end
		end.values
	end
