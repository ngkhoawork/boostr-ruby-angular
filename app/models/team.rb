class Team < ActiveRecord::Base
  ...
  def all_deals_for_time_period(start_date, end_date)
    deals.where(open: true).for_time_period(start_date, end_date) + children.map {|c| c.all_deals_for_time_period(start_date, end_date) } + (leader.nil? ? 0 : leader.all_deals_for_time_period(start_date, end_date))
  end
