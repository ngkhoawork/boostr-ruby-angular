...
class Deal < ActiveRecord::Base
  ...
  def disable_manual_deal_won_validation
    ...
    if validation && disable_flag == true && stage && stage.probability == 100 && stage.open? == false
      errors.add(:stage, "#{self.previous_stage.try(:name)} can't be updated to #{self.stage.try(:name)} manually. Deals can only be set to #{self.stage.try(:name)} from API integration")
    end
  end
