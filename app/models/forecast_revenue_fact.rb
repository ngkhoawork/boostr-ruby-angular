class ForecastRevenueFact < ActiveRecord::Base
  ...
  scope :by_time_dimension_id, -> (time_dimension_id) do
    where('forecast_revenue_facts.forecast_time_dimension_id = ?', time_dimension_id) if time_dimension_id
  end
  scope :by_user_dimension_ids, -> (user_dimension_ids) do
    if user_dimension_ids && user_dimension_ids.count > 0
      where('forecast_revenue_facts.user_dimension_id in (?)', user_dimension_ids)
    end
  end
