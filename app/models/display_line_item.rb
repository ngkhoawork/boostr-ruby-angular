  ...
  def self.import(file, current_user_id, file_path)
    ...
    CSV.parse(file, headers: true) do |row|
      ...
      if io_id.nil?
        ...
        io_change[:time_period_ids] += company.time_periods.where("end_date >= ? and start_date <= ?", io.start_date, io.end_date).collect{|item| item.id}
        ...
        io.save
        deal_change[:time_period_ids] += company.time_periods.where("end_date >= ? and start_date <= ?", io.deal.start_date, io.deal.end_date).collect{|item| item.id}
