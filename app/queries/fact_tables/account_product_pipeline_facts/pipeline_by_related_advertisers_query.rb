class FactTables::AccountProductPipelineFacts::PipelineByRelatedAdvertisersQuery
  ...
  module FactScopes
    ...
    def by_agencies(agencies_ids)
      where('advertiser_agency_pipeline_facts.agency_id in (:agencies_ids)', agencies_ids: agencies_ids)
    end

    def by_related_advertisers(advertisers_ids)
      where('advertiser_agency_pipeline_facts.advertiser_id in (:advertisers_ids)', advertisers_ids: advertisers_ids)
    end
