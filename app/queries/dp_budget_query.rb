class DpBudgetQuery
  ...
  def all
    @result ||= @relation.joins("INNER JOIN deal_products ON deal_product_budgets.deal_product_id = deal_products.id")
             ...
             .where("(deals.advertiser_id = ? OR deals.agency_id = ?) AND deals.company_id = ? AND deal_products.open IS TRUE", options[:client_id], options[:client_id], options[:company_id])
