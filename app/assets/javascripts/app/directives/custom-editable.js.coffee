@directives.directive 'clickToEdit', ($timeout) ->
  ...
  replace: true
  transclude: false
  template: '<div class="templateRoot">' + '<span class="editable" ng-show="!editState" ng-click="toggle()" ng-if="type==\'number\'">{{prefix}}{{model | number : 0}}{{postfix}}</span>' + '<span class="editable" ng-show="!editState" ng-click="toggle()" ng-if="type!=\'number\'">{{prefix}}{{model}}{{postfix}}</span>' + '<input class="inputText editable-field" type="text" ng-model="localModel" enter-press="toggle()" ng-show="editState && type == \'inputText\'" ng-blur="save()"/>' + '</div>'
