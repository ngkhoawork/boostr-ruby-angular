class Api::SalesExecutionDashboardController < ApplicationController
  ...
  def index
    ...
    probabilities.each do |probability|
      data = Deal.joins(:deal_products).joins(:products).at_percent(probability).where("products.company_id = ? and deals.id in (?)", current_user.company.id, deal_ids).group("products.id").order("products.id asc").select("products.name, sum(deal_products.budget / 100) as total_budget").collect {|deal| {label: deal.name, value: deal.total_budget}}
      final_data_weighted = []
