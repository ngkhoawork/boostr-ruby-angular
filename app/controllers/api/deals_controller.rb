      ...
      format.json {
        if params[:name].present?
          ...
          response_deals = response_deals.map do |deal|
            ...
            deal['month_amounts'] = []
            monthly_revenues = DealProductBudget.select("date_part('month', start_date) as month, (sum(budget)/100.0) as revenue").where("deal_id=? and date_part('year', start_date) = ?", deal['id'], params[:year]).group("date_part('month', start_date)").order("date_part('month', start_date) asc").collect {|deal| {month: deal.month.to_i, revenue: deal.revenue}}
            index = 0
