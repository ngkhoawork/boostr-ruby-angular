...
RSpec.describe Operative::ExtractVerifyService, datafeed: :true do
  ...
  let(:files) { ['./testfile_4148282_v3.tar.gz', './testfile_4148282_v3.csv'] }
  ...
  describe '#perform' do
    it 'requests to download files' do
      ...
      expect(tar_archive).to receive(:each)#.and_return([sales_orders, sales_order_items, invoice_items, extra_file].each)
      # expect(sales_orders).to receive(:full_name).and_return('Sales_Order_03052017.csv')
