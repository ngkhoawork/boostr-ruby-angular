...
describe Facts::AccountProductRevenueFactService do
  ...
  describe '#perform' do
    ...
    let(:io) { create(:io, advertiser: client, agency: nil, deal: deal, company: company) }
    ...
    let!(:account_dimension) { create(:account_dimension, id: client.id, name: client.name, company_id: client.company_id, account_type: 10) }
    ...
    context 'display line items without display line item budgets' do
      let(:io) { create(:io, advertiser: client, agency: nil, deal: deal, company: company) }
      let!(:display_line_item) { create(:display_line_item, product: display_product, io: io, start_date: Date.today, end_date: Date.today + 2.day, daily_run_rate: 1000) }
