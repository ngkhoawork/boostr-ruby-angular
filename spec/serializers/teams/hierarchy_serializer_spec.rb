...
describe Teams::HierarchySerializer do
  before do
    member1
    member2
    child_team1
  ...
  it 'serializes user data' do
    ...
    expect(serializer.all_leaders.count).to eq(1)
    expect(serializer.all_leaders).to include(leader_serializer)
    expect(serializer.all_members.count).to eq(2)
    expect(serializer.all_members).to include(member1_serializer)
    expect(serializer.all_members).to include(member2_serializer)
  ...
  def leader_serializer
    Users::BaseSerializer.new(leader).object
  end

  def member1_serializer
    Users::BaseSerializer.new(member1).object
  end

  def member2_serializer
    Users::BaseSerializer.new(member2).object
  end

  def child_team1_serializer
    described_class.new(child_team1).object
  end

  def child_team2_serializer
    described_class.new(child_team2).object
  end
