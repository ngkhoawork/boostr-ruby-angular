...
describe Forecast::PmpRevenueProductDataSerializer do
  before do
    pmp_item
    pmp_item2
    pmp_member
  ...
  it 'pmp revenue data serialized data' do
    ...
    expect(pmp_revenue_data.sum_period_budget).to eq(15000.0)
    expect(pmp_revenue_data.split_period_budget).to eq(9000.0)
    expect(pmp_revenue_data.products).to eq({
      0 => {
        product_id: nil, 
        product: nil, 
        in_period_amt: 15000.0,
        in_period_split_amt: 9000.0
      }
    })
  end
