...
RSpec.describe Revenue, type: :model do
  ...
  describe 'alerts' do
    ...
    it 'returns run rate' do
      ...
      expect(revenue.run_rate.to_i).to equal((((revenue.budget-revenue.budget_remaining)/(DateTime.now.to_date-revenue.start_date.to_date+1))*100).to_i)
      expect(revenue.remaining_day.to_i).to equal((revenue.budget_remaining/(revenue.run_rate/100)).to_i)
      expect(revenue.balance.to_i).to equal((((revenue.end_date.to_date-DateTime.now.to_date+1)-revenue.remaining_day)*(revenue.run_rate/100)).to_i)
      Revenue.set_alerts(company.id)
      user.reload
      expect(user.neg_balance_l_cnt.to_i).to equal(1)
      expect(user.pos_balance_l_cnt.to_i).to equal(0)
      expect(user.neg_balance_l.to_i).to equal((((revenue.end_date.to_date-DateTime.now.to_date+1)-revenue.remaining_day)*(revenue.run_rate/100)).to_i)
