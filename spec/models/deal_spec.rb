...
RSpec.describe Deal, type: :model do
  ...
  describe '#in_period_amt' do
    ...
    it 'returns the whole budget of a deal product when the deal product is wholly within the same time period' do
      create :deal_product_budget, deal: deal, start_date: '2015-01-01', end_date: '2015-01-31', budget: 100000

      expect(deal.in_period_amt(time_period.start_date, time_period.end_date)).to eq(1000)
    end

    it 'returns the whole budget of a deal product when the deal product is wholly within the same time period' do
      create :deal_product_budget, deal: deal, start_date: '2015-01-27', end_date: '2015-02-05', budget: 100000
  ...
  describe '#add_product' do
    ...
    it 'creates the correct number of DealProductBudget objects based on the deal timeline' do
      ...
      expect do
        ...
        expect(DealProductBudget.all.map(&:budget)).to eq(expected_budgets)
        expect(deal.budget).to eq(9_500_000)
      end.to change(DealProductBudget, :count).by(4)
    end
  ...
  describe '#remove_product' do
    let(:deal) { create :deal }
    let(:product) { create :product }
    let!(:deal_product_budget) { create :deal_product_budget, deal: deal, product: product, start_date: deal.start_date, end_date: deal.start_date.end_of_month }
