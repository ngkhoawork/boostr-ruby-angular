  ...
  context 'scopes' do
    ...
    context 'for_time_period' do
      ...
      it 'returns deals that are completely in the time period' do
        expect(DealProduct.for_time_period(time_period.start_date, time_period.end_date).count).to eq(1)
        expect(DealProduct.for_time_period(time_period.start_date, time_period.end_date)).to include(in_deal_product)
      end
      ...
      it 'returns deals that are partially in the time period' do
        ...
        expect(DealProduct.for_time_period(time_period.start_date, time_period.end_date).count).to eq(3)
      end
    end
  end
end
